...
class HittingHeightsLoss(FeasibilityLoss):
    def __init__(self, N, urdf_path, end_effector_constraint_distance_function, obstacle_distance_function, q_dot_limits, q_ddot_limits,
        ...
        super(HittingHeightsLoss, self).__init__(N, os.path.join(os.path.dirname(urdf_path), UrdfModels.iiwa),
    ...
    def call(self, q_cps, t_cps, data):
        ...
        constraint_loss = self.end_effector_constraints_distance_function(xyz, dt, data)
        #puck_pose = data[:, -2:]
        #obstacle_loss = self.obstacle_distance_function(xyz, dt, puck_pose)[:, tf.newaxis]
        t_loss = huber(t[:, tf.newaxis])
        obstacle_loss = tf.zeros_like(t_loss)
