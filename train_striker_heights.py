...
from losses.hittting_heights import HittingHeightsLoss
from utils.dataset import _ds
from utils.execution import ExperimentHandler
from losses.constraint_functions import air_hockey_table, air_hockey_puck, air_hockey_table_heights
from losses.hittting import HittingLoss
from models.iiwa_planner_boundaries import IiwaPlannerBoundariesHitting, IiwaPlannerBoundariesHittingHeights
...
class args:
    ...
    log_interval = 100
    learning_rate = 5e-5
    dataset_path = "./datasets/air_hockey_hitting_heights/train/data.tsv"
...
loss = HittingHeightsLoss(N, urdf_path, air_hockey_table_heights, air_hockey_puck, Limits.q_dot, Limits.q_ddot, Limits.q_dddot, Limits.tau)
model = IiwaPlannerBoundariesHittingHeights(N, 3, 2, loss.bsp, loss.bsp_t)
...
for epoch in range(30000):
    ...
    for i, d in _ds('Train', dataset_epoch, train_size, epoch, args.batch_size):
        with tf.GradientTape(persistent=True) as tape:
            ...
            z_loss_abs = np.mean(np.abs(xyz[..., -1, 0] - d[:, -1:]), axis=-1)
