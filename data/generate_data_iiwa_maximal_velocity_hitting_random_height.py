...
if __name__ == "__main__":
    ...
    while i < N:
        ...
        table_height = (thh - thl) * np.random.rand() + thl
        point = np.array([x0, y0, table_height])
        q0 = po.solve(point)
        q_dot_0 = np.zeros((6))
        q_ddot_0 = np.zeros((6))
        ...
        qk, q_dot_k, vk = get_hitting_configuration(xk, yk, thk, q0.tolist(), zk=table_height)
        ...
        data.append(q0.tolist() + qk + [xk, yk, thk] + q_dot_0.tolist() + [0.] + q_ddot_0.tolist() + [0.] + q_dot_k.tolist() + [table_height])
        i += 1
